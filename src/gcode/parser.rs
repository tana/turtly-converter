...
use nom::IResult;
use nom::character::complete::{digit0, digit1, alpha1, char, space0, not_line_ending};
use nom::{branch::alt, sequence::{tuple, pair, separated_pair, preceded, terminated}, combinator::{opt, recognize, map}};
...
pub(crate) fn parse_float_arg(input: &str) -> IResult<&str, f32> {
    ...
    ))), |s| f32::from_str(s).unwrap())(input)  // from_str will succeed
}
